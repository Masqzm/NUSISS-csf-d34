...
//@Injectable({ providedIn: 'root' })   // similar to @Service
@Injectable()   // if providedIn not declared, must declare svc in module -> providers
export class DogService {
    // RestTemplate
    private http = inject(HttpClient)

    newDogSearch = new Subject<string[]>()  // to pass result to other components thru this service
    ...
    getDogsAsStringArr(count = 3): Promise<string[]> {
        // Convert Observable -> Promise
        ...
        return firstValueFrom(
            ...
            this.newDogSearch.next(result.message)  // to pass result to other components thru this service
